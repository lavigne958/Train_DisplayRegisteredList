...
HeaderSelector::HeaderSelector(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::HeaderSelector)
{
    this->updateInProgress = false;
    ui->setupUi(this);
}

HeaderSelector::HeaderSelector(QWidget *parent, QStringList headers):
    QDialog(parent),
    ui(new Ui::HeaderSelector),
    headers(headers)
{
    ...
    connect(this->ui->okCancelButtons, &QDialogButtonBox::clicked,
            this, &HeaderSelector::on_ok_button_click);
    this->updateInProgress = false;
...
QComboBox *
HeaderSelector::addUiHeaderChoice(int pos)
{
    ...
    connect(headersChoice, QOverload<const QString &>::of(&QComboBox::currentIndexChanged),
            this, &HeaderSelector::on_choice_changed);

    return headersChoice;
}

void
HeaderSelector::stripHeader(int maxStrSize)
{
    for (int i = 0; i < this->headers.size(); ++i) {
...
void
HeaderSelector::on_choice_changed(const QString &newText)
{
...
void
HeaderSelector::on_ok_button_click(QAbstractButton *button __attribute__((unused)))
{
